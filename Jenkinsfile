...
pipeline {
    agent any
    environment {
        DOCKER_IMAGE = 't0ky0le/project-service:1.0' // Nombre de la imagen en Docker Hub
    }
    stages {
        stage('Checkout') {
            steps {
                echo 'Clonando el repositorio...'
                git branch: 'main', url: 'https://github.com/TokyoF/project-service-taskflow.git' // Cambia con tu URL
            }
        }
        stage('Build') {
            steps {
                echo 'Construyendo el proyecto con Maven...'
                bat './mvnw.cmd clean package -DskipTests' // Compila el proyecto
            }
        }
        stage('Docker Build') {
            steps {
        ...
        stage('Docker Compose Down') {
            steps {
                ...
                bat 'docker rm -f project-service || true' // Elimina el contenedor si existe
